...
# Load the dataset.
traces = pd.read_csv('../data/csv/heriarchical_logistic_regression_traces.csv')
data = pd.read_csv('../data/csv/compiled_data.csv')

# Split by the shock rate.
slow_data = data[(data['class'] == 'shock') & (data['flow_rate'] < 1)].copy()
fast_data = data[(data['class'] == 'shock') & (data['flow_rate'] >= 1)].copy()
...
for s in samples:
    ...
    alpha, beta = np.median(
        samp['alpha'].values), np.median(samp['beta'].values)
    prob_survival[s] = (1 + np.exp(-alpha - beta * chan_range))**-1
    _cred = np.zeros((2, len(chan_range)))

    # Compute the credible regions.
    alpha, beta = samp['alpha'].values, samp['beta'].values
    for i, c in enumerate(chan_range):
        _prob = (1 + np.exp(-alpha - beta * c))**-1
...
for j, exp in enumerate([slow_data, fast_data]):
    ...
    for i in (0, 1):
        ...
        _ = a.plot(_g['chan_per_cell'], _y, 'k.', ms=1.5, alpha=0.2)
