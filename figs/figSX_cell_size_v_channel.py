...
shock_data = data[data['class'] == 'shock']

# Look only at MLG 910 for channel counts.
mlg910 = data[data['rbs'] == 'MLG910']

CAL = 348 / np.mean(mlg910['area'] * mlg910['rescaled_intensity'])
...
for g, d in grouped:
    _ = ax[0].plot(d['area'], d['area'] * d['rescaled_intensity'] * CAL, '.',
                   color=color_dict[g], ms=4, alpha=0.5, label=label_dict[g])
    _ = ax[1].plot(d['area'], d['chan_per_cell'], '.', color=color_dict[g],
