...
dirs = [
    '{0}autofluorescence/xy*'.format(root_dir)]


_dfs = []
for d in dirs:
    sample_positions = glob.glob(d)
...
auto_df = pd.concat(_dfs, axis=0)

# Compute the mean autofluorescence value.
mean_auto = np.mean(auto_df['birth_fluo'] / auto_df['birth_area'])
...
bg = bleach['bg'].values[0]
tau_1 = bleach['tau_1'].values[0]
tau_2 = bleach['tau_2'].values[0]
beta_1 = bleach['beta_1'].values[0]
beta_2 = bleach['beta_2'].values[0]
...
dilution_df.loc[:, 'corrected_fluo'] = (dilution_df.loc[:, 'death_fluo'] -
                                        (mean_auto * dilution_df.loc[:, 'death_area'])) / bleaching_fraction
