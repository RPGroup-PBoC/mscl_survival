...
for i in (0, 1, 4, 5):
    if (i == 4) or (i == 5):
        ...
        ax[i].set_xlabel('effective channel number', fontsize=10)
...
# for i in range(2):
_leg = ax[2].legend(fontsize=10, loc='lower right', handlelength=1, bbox_to_anchor=(2.2, -0.5), ncol=3)
plt.setp(_leg.get_title(), fontsize=10)
...
for i in (2, 3):
    ...
    ax[i].set_ylabel('survival probability', fontsize=10)
