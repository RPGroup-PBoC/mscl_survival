...
fig.text(0.6, 0.95, '(B)', fontsize=10)
fig.text(0.6, 0.5, '(C)', fontsize=10)
# Plot the KDE if the samples.
_ = sns.kdeplot(data['hyper_A_mu'], data['hyper_alpha_mu'], ax=ax0, shade=True,
cmap=plt.cm.Reds)
ax0.set_ylabel('calibration factor [a.u. / MscL channel]', fontsize=10)
ax0.set_xlabel('average cell area [µm$^2$]', fontsize=10)


# Add a fake legend
ax1.plot([], [], color=colors['blue'], lw=3, label='replicate\n parameter')
ax1.plot([], [], color=colors['red'], lw=3, label='hyper-\nparameter')
ax1.legend(fontsize=10, handlelength=1, bbox_to_anchor=(1.3, 1))
...
ax1.set_xlabel('calibration factor [a.u./channel]', fontsize=10)
ax2.set_xlabel('average cell area [µm$^2$]', fontsize=10)
ax1.set_ylabel('$\propto$ probability', fontsize=10)
ax2.set_ylabel('$\propto$ probability', fontsize=10)
for a in (ax1, ax2):
    a.set_yticks([])
