...
_ = sns.boxplot(data['rbs'], data['scaled_intensity'] / data[data['rbs'] == 'mlg910']['scaled_intensity'].mean(), 
                order=intensity_order, fliersize=0, linewidth=0.75, palette='Greens', ax=ax[0])
_ = sns.stripplot(data['rbs'], data['scaled_intensity'] / data[data['rbs'] == 'mlg910']['scaled_intensity'].mean(), 
                  order=intensity_order, jitter=True, marker='.', size=2.5, alpha=0.3, color='k', ax=ax[0])
ax[0].vlines(1, 0, 3, color=colors['pale_yellow'], lw=20, zorder=0, alpha=0.75)
# ax[1].vlines(1, 0, 1000, color=colors['pale_yellow'], lw=20, zorder=0, alpha=0.75)
...
_ = sns.boxplot('rbs', 'effective_channels', data=shock_data,
                order=[i for i in channel_order if i != 'mlg910'], fliersize=0, linewidth=0.75,
                palette='Greens', ax=ax[1])
_ = sns.stripplot('rbs', 'effective_channels', data=shock_data,
                  order=[i for i in channel_order if i != 'mlg910'], jitter=True, marker='.', size=2.5,
                  alpha=0.3, color='k', ax=ax[1], zorder=1001)


# Add the marker for the standard candle strain
ax[0].set_ylim(0, 3)
ax[1].set_ylim(0, 1000)
ax0_labels = [i.upper() for i in intensity_order]
ax1_labels = [i.upper() for i in intensity_order if i != 'mlg910']
...
ax[0].set_xlabel('Shine-Dalgarno modification', fontsize=10)
ax[1].set_xlabel('Shine-Dalgarno modification', fontsize=10)
